...
import MyComplaintsList from '../complaints/MyComplaintsList';
...
export default function StudentDashboard() {
  ...
  const navItems = [
    ...
    { id: 'raise', label: 'Raise', icon: PlusCircle },
    { id: 'history', label: 'Complaints', icon: ListOrdered },
        ...
        <div className="pt-4 mt-6 border-t border-white/10 space-y-3">
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[11px] text-sidebar-text truncate">Student Account</p>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto pb-16 md:pb-0">
        ...
        <header className="h-16 bg-white border-b border-surface-border flex items-center justify-between px-4 md:px-6 sticky top-0 z-10 flex-shrink-0 shadow-subtle">
          ...
          <div className="flex items-center gap-3 relative">
            ...
            <div
              onClick={() => handleTabChange('settings')}
              className="w-8 h-8 rounded-full bg-brand text-white flex items-center justify-center font-display font-bold text-xs cursor-pointer hover:opacity-90 transition"
              title="Settings"
            >
              {user?.name ? user.name.slice(0, 2).toUpperCase() : 'ST'}
            </div>
              ...
              {currentTab === 'raise' && (
                <RaiseComplaint
                  onComplaintSubmitted={(newComplaint) => {
                    if (newComplaint?.ticketId) {
                      setSelectedComplaintId(newComplaint.ticketId);
                    } else {
        ...
        {/* MOBILE FLOATING ACTION BUTTON (FAB) FOR STUDENT RAISE COMPLAINT */}
        {currentTab !== 'raise' && !selectedComplaintId && (
          <button
            onClick={() => handleTabChange('raise')}
            className="fixed bottom-20 right-5 w-13 h-13 rounded-full bg-brand text-white flex items-center justify-center shadow-lg shadow-brand/35 z-30 md:hidden hover:scale-105 active:scale-95 transition cursor-pointer"
